...
from odc.stats.io import S3COGSink
from odc.stats.model import Task, TaskResult, TaskRunnerConfig
from odc.stats.plugins import _plugins
...
class TaskRunner:
    def __init__(
        ...
        # TODO: avoid the _plugin.resolve,
    ...
    def _run(self, tasks: Iterable[Task]) -> Iterator[TaskResult]:
        ...
        for task in tasks:
            ...
            for (dt, t_now) in wait_for_future(cog, cfg.future_poll_interval, t0=t0):
                ...
                if 0 < cfg.max_processing_time < dt:
                    _log.error(
