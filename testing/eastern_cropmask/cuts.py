...
for train_index, test_index in outer_cv.split(X, y, groups=spatial_groups):
    print(f"Working on {i}/5 outer cv split\r")
    ...
    clf = GridSearchCV(
        ...
        n_jobs=ncpus,
        refit=True,
        cv=inner_cv.split(X_tr, y_tr, groups=spatial_groups[train_index]),
    )
...
selected_features = selector.fit_transform(
    model_input[:, model_col_indices], model_input[:, 0]
)
...
model_input = np.append(
    model_input[:, 0].reshape(len(model_input[:, 0]), 1), selected_features, axis=1
)
...
plt.xticks(
    ticks=range(len(lda_table)), labels=lda_table["name"], rotation=70, ha="right"
)
...
with HiddenPrints():
    ds = load_ard(dc=dc, products=products, dask_chunks=dask_chunks, **query)
    ...
    mads.to_netcdf(
        results + "input/annual_mads/Eastern_tile_" + g_id + "_annual_mads.nc"
    )
...
noncrop_X, noncrop_y = outlier_detect(
    noncrop_X, noncrop_y, OneClassSVM, nu=outlier_percent
)
...
# convert tiff to kea
gdal.Translate(
    destName=kea_file, srcDS=results + tif_to_seg, format="KEA", outputSRS="EPSG:6933"
)
...
zonal_stats_parallel(
    shp=results + tif_to_seg[:-4] + "_segmented.shp",
...
def gm_two_seasons_mads_annual_training(ds):
    ...
    result = xr.merge([epoch1, epoch2, mads, slope], compat="override")
